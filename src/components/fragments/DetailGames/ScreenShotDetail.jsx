...
const ScreenShootDetail = (props) => {
  const { publisher = [], image = [],imageContentRef } = props;
      ...
      >
        {image.length > 0 &&
          image.map((img) => (
            <div
              key={img.id}
              className="w-4/5 h-44 bg-sunset box-shadow border-4 border-lightSunset flex-shrink-0 overflow-hidden mr-2 md:w-3/5 md:h-56 lg:w-1/2 lg:h-64 xl:w-2/5 xl:h-72"
            >
              <img
                src={img.image}
                alt={img.image}
